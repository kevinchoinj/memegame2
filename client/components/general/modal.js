...
const StyledWrapper = styled.div`
  ...
  left: 0;
  top: 0;
  display: ${(props) => (props["data-displayed"] ? "flex" : "none")};
...
const StyledMask = styled.div`
  ...
  opacity: ${(props) => (!props["data-displayed"] ? 0 : 1)};
  pointer-events: ${(props) => !props["data-displayed"] && "none"};
...
const StyledModal = styled.div`
  ...
  position: relative;
  border-radius: 12px;
  width: ${(props) => props["data-width"] || "440px"};
  h2 {
    margin: 0 0 18px 0;
    font-size: 24px;
  }
